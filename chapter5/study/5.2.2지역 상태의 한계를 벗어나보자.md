...
2. 각 컴포넌트가 비정상적 작동. 다른쪽은 리렌더링 되지 않고 버튼을 눌러야만 최신값 불러옴. 다른쪽의 리렌더링을 일으킬 무언가가 없기 때문.
...
2. 컴포넌트로 넘겨받는 콜백 함수 저장 위해 callbacks를 Set으로 선언. Set은 윈시값이나 객체에 관계없이 유일한 값 저장할 수 있어 중복 없이 콜백 함수 저장하기 유용
...
=> 원하는 값이 변했을 때만 리렌더링 되도록 useStore 훅 수정
